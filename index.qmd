...
This interactive map displays data from **Section 10 - Shelter, Table 10.2 - Estimated Hazard Damage**, showing the estimated number of private households damaged by the cyclone across Vanuatu's Area Councils. **Hover over each council** to view detailed damage statistics including roof and wall materials affected. Councils shown in grey have no damage data available.
...
# Define province names to exclude
provinces <- c("Malampa", "Penama", "Sanma", "Shefa", "Tafea", "Torba")

# Load Area Council boundaries
councils_sf <- st_read(here::here("Data", "GIS_layers", "2016_phc_vut_acid_4326.geojson"), quiet = TRUE)

# Prepare damage data for mapping - include ALL variables
map_data <- shelter_damage_aggregated %>%
  filter(!Region %in% c("National", provinces))

# Join spatial data with damage data
councils_map <- councils_sf %>%
  left_join(map_data, by = c("acname" = "Region"))
```

````{r damage-map, fig.height=8, fig.width=10}
# Create color palette
pal <- colorNumeric(
  palette = "YlOrRd",
  domain = councils_map$private_households_damaged,
  na.color = "#cccccc"
)

# Create interactive map
leaflet(councils_map) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(
    fillColor = ~pal(private_households_damaged),
    fillOpacity = 0.7,
    color = "#333333",
    weight = 1,
    ...
    label = ~paste0(
      ...
      "<strong>Private Households Damaged:</strong> ", format(round(private_households_damaged, 0), big.mark = ","), "<br>",
      "<br>",
      "<strong>Main Roof Materials Damaged</strong><br>",
      "Concrete: ", format(round(roof_concrete_damaged, 0), big.mark = ","), "<br>",
      "Metal: ", format(round(roof_metal_damaged, 0), big.mark = ","), "<br>",
      "Wood: ", format(round(roof_wood_damaged, 0), big.mark = ","), "<br>",
      "<br>",
      "<strong>Main Wall Materials Damaged</strong><br>",
      "Concrete: ", format(round(wall_concrete_damaged, 0), big.mark = ","), "<br>",
      "Metal: ", format(round(wall_metal_damaged, 0), big.mark = ","), "<br>",
      "Wood: ", format(round(wall_wood_damaged, 0), big.mark = ",")
  ...
  addLegend(
    ...
    pal = pal,
    values = ~private_households_damaged,
    title = "Households<br>Damaged",
    opacity = 0.7,
