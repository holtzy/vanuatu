...
This file runs based on an excel sheet `1b Input- Ex Hazard Areas`, which is the sheet 2 of `11d. Full Data Pipeline.xlsx` and will serve as our config file for later analyses. This file provides the cyclone intensity applied to each council in Vanuatu. It has 5 columns:
- National
- Province
- Area Council
- Hazard
...
```{r}
config <- read_excel(here::here("Data", "11d. Full Data Pipeline.xlsx"), sheet = 2)
```
...
```{r}
# Read the Excel sheet called "1c Input - Baseline"
full_data <- read_excel(here("data", "11d. Full Data Pipeline.xlsx"), sheet = 3)
...
resources_needed <- education_with_config_resources %>%
  filter(!is.na(category)) %>%  # Only calculate for councils with assigned categories
  rowwise() %>%
  mutate(
    # Get damage multiplier for this council's specific cyclone category
    damage_multiplier = get_damage_multiplier(category, "ecce", "schools"),
