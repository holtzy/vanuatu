...
# We need population data for calculating some resources
# Get total population from Gender & Protection baseline
population_data <- full_data %>%
  filter(Baseline == "Gender & Protection") %>%
  filter(Year == max(Year, na.rm = TRUE)) %>%
  filter(!is.na(`Area Council`)) %>%
  filter(Indicator == "Population Sex") %>%
...
# Add cyclone categories and population to health damage data
health_resources_base <- damage_estimates_raw %>%
  left_join(config, by = c("Region" = "Area Council")) %>%
  left_join(population_data, by = "Region")

# Calculate resources needed
resources_needed <- health_resources_base %>%
  filter(!is.na(Intensity)) %>%
  rowwise() %>%
  mutate(
    # Calculate total damaged facilities for resource allocation
    total_facilities_damaged = facility_hospital_damaged + `facility_health centre_damaged` + 
                                facility_dispensary_damaged + facility_aidpost_damaged,
    
    # Calculate affected population using damage multiplier
    damage_multiplier = get_health_damage_multiplier(Intensity, "Health Facility", "hospital", Region),
    affected_population = round(total_population * damage_multiplier, 0),
    
    # Resources based on damaged facilities and affected population
    medical_tent = round(total_facilities_damaged * get_health_resource_multiplier("Medical Tent", Region), 0),
    emergency_health_kits = round(affected_population * get_health_resource_multiplier("Emergency Health Kits", Region), 0),
    trauma_surgical_kits = round(total_facilities_damaged * get_health_resource_multiplier("Trauma and Surgical Kits", Region), 0),
    essential_ncd_medicines = round(affected_population * get_health_resource_multiplier("Essential NCD Medicines", Region), 0),
    mosquito_nets = round(affected_population * get_health_resource_multiplier("Mosquito Nets", Region), 0)
  ) %>%
  ungroup() %>%
...
reactable(
  ...
  columnGroups = list(
    colGroup(name = "Region", columns = c("Region")),
    colGroup(name = "Health Facility", columns = c("total_financial_damage", "hospital_financial", 
