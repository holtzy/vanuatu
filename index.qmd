...
# NOTE FOR THEO: I think those values are already available in the initial file. education_data has them on top
...
calculate_damage <- function(region_data) {
  region_data %>%
    rowwise() %>%
    mutate(
      # Use region-specific category (default to 0 if NA)
      cyclone_cat = ifelse(is.na(category), 0, category),
      
      ECCE_Schools_Damaged = round(ECCE_Schools * get_damage_multiplier(cyclone_cat, "ecce", "Schools")),
      ECCE_Students_Affected = round(ECCE_Students * get_damage_multiplier(cyclone_cat, "ecce", "Students")),
      
      Primary_Schools_Damaged = round(Primary_Schools * get_damage_multiplier(cyclone_cat, "primary", "Schools")),
      Primary_Students_Affected = round(Primary_Students * get_damage_multiplier(cyclone_cat, "primary", "Students")),
      
      Secondary_Schools_Damaged = round(Secondary_Schools * get_damage_multiplier(cyclone_cat, "secondary", "Schools")),
      Secondary_Students_Affected = round(Secondary_Students * get_damage_multiplier(cyclone_cat, "secondary", "Students"))
    ) %>%
    ungroup() %>%
