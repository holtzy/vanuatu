...
# Calculate resources needed based on FACILITIES with FIXED multipliers
resources_needed <- health_facilities_damaged %>%
  rowwise() %>%
  mutate(
    # ALL resources based on damaged facilities with fixed multipliers
    medical_tent = round(1 * total_facilities_damaged, 0),
    emergency_health_kits = round(5 * total_facilities_damaged, 0),
    trauma_surgical_kits = round(5 * total_facilities_damaged, 0),
    essential_ncd_medicines = round(1 * total_facilities_damaged, 0),
    mosquito_nets = round(20 * total_facilities_damaged, 0)
  ) %>%
  ungroup() %>%
