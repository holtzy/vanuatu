...
# The damage_estimates table provides values for concils only. 
# We need to aggregate values per province and country
province_damage <- damage_estimates %>%
  # remove bolding for now so we can group cleanly
  mutate(Region = gsub("<b>|</b>", "", Region_Formatted)) %>%
  left_join(education_wide %>% select(Region, Province), by = "Region") %>%
  group_by(Province) %>%
  summarise(across(ends_with("Damaged") | ends_with("Affected"), sum, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(
    Region_Formatted = paste0("<b>", Province, "</b>")  # bold for display
  )

# Step 3. National-level damages (sum of provinces)
national_damage <- province_damage %>%
  summarise(across(ends_with("Damaged") | ends_with("Affected"), sum, na.rm = TRUE)) %>%
  mutate(Region_Formatted = "<b>Vanuatu</b>")

# Step 4. Combine all levels
damage_estimates <- bind_rows(
  national_damage,
  province_damage,
  council_damage
)
